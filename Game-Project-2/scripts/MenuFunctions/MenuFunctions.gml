...
function MenuSelectAction(_user,_action)
{
	...
	//activating the targetting cursor if needed, or simply begin the action
	with(oBattle) 
	{
		if(_action.targetRequired)
		{
			with(cursor)
			{
				...
				if(targetAll == MODE.VARIES) targetAll=true;//"toggle" as all by default
				activeUser=_user;
		...
		{
			//If no target needed, begin tha action and end the menu
